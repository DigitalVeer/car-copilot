...
- **Data layer**: `OBDDataSource` interface implemented by three sources selected at build time via `-PdataSource=FIXTURE|EMULATOR|BLUETOOTH` (`BuildConfig.DATA_SOURCE`).
  - `FixtureOBDDataSource` reads one of the bundled scenarios — `misfire.json` (P0301 2021 VW Jetta 1.4 TSI) or `hilux_fuel_rail.json` (P0087 Hilux diesel) — selected by the `ACTIVE_FIXTURE` const in the file. One-line edit to switch.
...
- **Performance** (Phase 8 + 2026-05-18): `Engine` initializes in `appScope.async` at process start; a parallel `prewarmJob` sends a dummy "ok" turn and cancels at first token. Conversation slot hoisted across calls for KV-cache reuse. ~24% first-token latency win on Pixel 9 from prewarm alone. **2026-05-18 additions:** capability-gated speculative decoding (Multi-Token Prediction) via `ExperimentalFlags.enableSpeculativeDecoding`, guarded by a runtime `Capabilities(modelPath).hasSpeculativeDecodingSupport()` probe — measured +23–60% decode TPS depending on surface; `EngineConfig.maxNumTokens = 4096` to cap the working window; `EngineConfig.cacheDir = context.cacheDir.absolutePath` for second-load speedup; `ExperimentalFlags.enableBenchmark = true` plus a per-surface `bench surface=… …` log line carrying SDK-authoritative `init / ttft / prefill_tokens / decode_tokens / prefill_tps / decode_tps`. BenchmarkInfo data confirmed prefill (not decode) is the wall-clock bottleneck on every surface — every prompt token shaved is roughly as valuable as the entire MTP win. Full writeup in `reference/perf_notes_2026-05-18.md`.
...
- **Per-step phase splitting** (2026-05-18): the walkthrough-step prompt no longer inlines the full curated procedure document. `PromptBuilder.splitProcedureIntoPhases` parses each `assets/walkthroughs/<CODE>.md` at `## Phase N` headers at PromptBuilder construction; `phaseForStep(phases, stepNumber, totalSteps)` returns intro + indexed phase per step (step 1 → first phase, last step → last phase, middle by index, clamped). Measured ~21% prefill drop per step, ~5–6s wall-clock each. The walkthrough plan call still receives the full document — it clusters phases into the step list and needs the whole text.
- **Plan parser brace tolerance** (2026-05-18): `WalkthroughPlanState.balanceJsonTail` walks the assembled buffer with string-aware brace/bracket tracking and closes any unclosed structure at the tail. Captured live failure: E4B emitted a structurally valid plan ending in `}]` but no outer `}`; the old `tolerantPlanParse` used `s.lastIndexOf('}')` which also discarded the trailing `]` and silently fell back to canned `DTCEntry.walkthroughSteps` text — meaning the Gemma per-step path never exercised in practice. Without this fix the phase-split change above would be unobservable.
- **JVM unit test suite** (`./gradlew test`, 179 tests): coverage spans the streaming JSON extractors (`SynthesisStateTest`, `MechanicDraftStateTest`, `HistoryPatternStateTest`, `WalkthroughPlanStateTest`, `WalkthroughStepStateTest`), the procedure phase splitter (`ProcedurePhaseSplitTest`), the OBDSnapshot schema (`OBDSnapshotTest`), the snapshot-to-Issue builder (`IssueBuilderTest`), the DTC table contract (`DTCTableTest`), the deterministic classifier (`RulesEngineTest`), and the ELM327 protocol layer (`Elm327ProtocolTest`). The plan-parser tests inside `WalkthroughPlanStateTest` also cover the brace-balancing tolerance cases captured live from the Pixel 9 (missing outer `}`, mid-string truncation, trailing structural comma). The on-device Gemma smoke test (`app/src/androidTest/.../GemmaSmokeTest.kt`) is a separate lane gated on a model push to `/data/local/tmp/`. The emulator carries its own stdlib `unittest` suite at `emulator/test_obd_emulator.py`.
...
- `ui/SynthesisState.kt` including `extractSynthesisInProgress(buffer: String)` — the JSON-aware streaming text extracto
...
| Performance / engine-config notes | `reference/perf_notes_2026-05-18.md` (current ceiling: synthesis ~6–7 tok/s decode, walkthrough step ~9 tok/s avg; prefill is the dominant cost) |
| Prompt assembly + RAG retrieval | `app/src/main/java/com/example/carcopilot/inference/PromptBuilder.kt` |
| Per-step procedure splitting | `PromptBuilder.splitProcedureIntoPhases` / `phaseForStep` (file-scope free functions); tests in `app/src/test/java/com/example/carcopilot/inference/ProcedurePhaseSplitTest.kt` |
...
- **LLM SDK:** `com.google.ai.edge.litertlm:litertlm-android:0.11.0`. Package `com.google.ai.edge.litertlm.*` — `Engine`, `EngineConfig`, `Backend`, `Conversation`, `ConversationConfig`, `SamplerConfig`, `Message`, `Contents`, plus the experimental surface `ExperimentalFlags`, `Capabilities`, `BenchmarkInfo` (all behind `@OptIn(ExperimentalApi::class)`, opted in file-scope at the top of `GemmaService.kt`). `EngineConfig` sets `maxNumTokens = 4096` and `cacheDir = context.cacheDir.absolutePath`. `ExperimentalFlags.enableSpeculativeDecoding` flips on when `Capabilities(modelPath).hasSpeculativeDecodingSupport()` returns true at init; `ExperimentalFlags.enableBenchmark = true` unconditionally so the per-surface `bench surface=…` log line carries real numbers. (Replaces the older `com.google.mediapipe.tasks.genai.llminference` namespace, which still exists but is deprecated for the LiteRT-LM rebrand.)
...
- **Streaming:** `Conversation.sendMessageAsync(String): Flow<Message>` emits **per-token deltas** (not cumulative — confirmed empirically in Phase 5 Checkpoint A). Compose collectors append, never replace. Steady-state was ~5 tok/s after ~6s first-token latency on Pixel 9 at Phase 8; with MTP enabled (see Current state) decode now runs 6–10 tok/s depending on surface, and the SDK-reported time-to-first-token in `BenchmarkInfo` runs ~3–16s mostly dominated by prefill, not model warmup. The streaming UIs extract the in-progress content from a partial JSON buffer via per-surface extractors that are JSON-aware and never let a stray backslash or half-finished escape reach the user. The walkthrough plan parser additionally balances unclosed braces/brackets at the tail so cancelled-mid-stream or model-stopped-early output still parses instead of bouncing to canned fallback — see `WalkthroughPlanState.balanceJsonTail`.
