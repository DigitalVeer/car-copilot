...
**Shipped 2026-06-22 — MTP draft-acceptance tuning.** E2B was tried and rejected on voice (it is a genuinely smaller model — lossy). MTP is the lossless alternative: the E4B target verifies every drafted token, so output is identical to plain E4B and only decode speeds up. perf_notes 2026-05-18 flagged the default top-p 0.95 as the likely reason MTP landed at +23–60% rather than the documented >2×: a wide sampler makes the target sample more randomly, rejecting more drafts. `GemmaService.MTP_NARRATIVE_SAMPLER` (top-p 0.9, temp 0.2) now backs the mechanic-draft, history, and walkthrough-plan surfaces to raise draft acceptance; the synthesis wow-moment voice stays on `DEFAULT_SAMPLER` (top-p 0.95, temp 0.3) untouched. **Not yet measured on device** — confirm the `bench surface=… decode_tps` delta on draft/history/plan and eyeball the voice before extending the tighter sampler to synthesis. A silent-off guard (`Log.w "MTP unavailable …"`) now fires when the staged model carries no draft head, the failure mode where MTP yields zero speedup invisibly.
